...
def check_ampere_availability(compartment_id: str, availability_domain: str) -> bool:
    ...
    command = [
        ...
        "create-compute-capacity-report",
        "--availability-domain",
        availability_domain,
        "--compartment-id",
        compartment_id,
            ...
            [
                {
                    ...
                    "instance-shape-config": {"ocpus": 1.0, "memory-in-gbs": 6.0},
                }
            ]
        ),
    ]
...
def get_tenancy_id() -> str:
    ...
    try:
        subprocess.run(
            ["oci", "iam", "region", "list"], capture_output=True, text=True, check=True
        )
        # If this works, get the actual tenancy
        subprocess.run(
            ["oci", "setup", "config"], capture_output=True, text=True, check=True
        )
