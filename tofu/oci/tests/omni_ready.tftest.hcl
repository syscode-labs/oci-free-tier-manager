...
run "omni_ready_accepts_schemeful_endpoint" {
  ...
  variables {
    omni_endpoint = "https://omni.REDACTED_TAILNET_DOMAIN"
  }

  assert {
    condition     = strcontains(base64decode(oci_core_instance.ampere_instance[0].metadata["user_data"]), "apiUrl: \"https://omni.REDACTED_TAILNET_DOMAIN?jointoken=test-join-token\"")
