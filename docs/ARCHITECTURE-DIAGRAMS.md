...
graph TB
    subgraph "Phase 0: Setup"
        A[task setup] --> B[OCI CLI Config]
        B --> C[SSH Keys Generated]
        C --> D[terraform.tfvars Created]
        A --> E[task setup:flux]
        E --> F[Age Key Generated]
        F --> G[SOPS Configured]
        G --> H[Flux Manifests Ready]
    end
    
    subgraph "Phase 1: Image Building"
        I[task build:images] --> J[Dagger Pipeline]
        J --> K[Base Hardened Image<br/>Debian 12 + SSH + Tailscale]
        J --> L[Proxmox Image<br/>Base + Proxmox + Ceph]
        K --> M[Validate < 20GB]
        L --> M
        M --> N[Upload to OCI Storage]
    end
    
    subgraph "Phase 2: Infrastructure"
        O[task deploy:oci] --> P[3x Ampere A1<br/>1x Micro Bastion]
        P --> Q[VCN + Networking]
        Q --> R[Reserved IPs]
        R --> S[Tailscale Mesh]
    end
    
    subgraph "Phase 3: Proxmox"
        T[task deploy:proxmox] --> U[Form Cluster<br/>3 nodes]
        U --> V[Configure Ceph<br/>Distributed Storage]
        V --> W[Deploy Tailscale LXC]
        W --> X[Test VM Migration]
    end
    
    subgraph "Phase 4: Kubernetes"
        Y[task deploy:talos] --> Z[Create Talos VMs<br/>on Proxmox]
        Z --> AA[Bootstrap K8s Cluster]
        AA --> AB[Deploy Cilium CNI]
        AB --> AC[Bootstrap Flux GitOps]
        AC --> AD[Inject SOPS Keys]
    end
    
    subgraph "Phase 5: Applications"
        AE[Flux Reconciliation] --> AF[Tailscale Operator]
        AE --> AG[Cert-Manager]
        AE --> AH[OCI CCM]
        AE --> AI[Grafana Alloy]
    end
...
Detailed breakdown showing all steps, tools, and validations.

```mermaid
graph TB
    subgraph "Setup Phase"
        A1[scripts/setup.sh] --> A2{OCI CLI<br/>Configured?}
        A2 -->|No| A3[oci setup config]
        A2 -->|Yes| A4{SSH Keys<br/>Exist?}
        A3 --> A4
        A4 -->|No| A5[ssh-keygen -t rsa -b 4096<br/>~/.ssh/oci_key]
        A4 -->|Yes| A6[Read OCI Config]
        A5 --> A6
        A6 --> A7[Generate terraform.tfvars<br/>from OCI config]
        A7 --> A8[Prompt for Budget Email]
        
        B1[scripts/setup-flux.sh] --> B2[Generate Cilium Values]
        B2 --> B3[helm template cilium<br/>→ bootstrap/cilium.yaml]
        B3 --> B4[age-keygen -o age-key.txt]
        B4 --> B5[Create .sops.yaml<br/>with Age public key]
        B5 --> B6[Prompt for Tailscale OAuth]
        B6 --> B7[sops --encrypt<br/>tailscale-secret.yaml]
        B7 --> B8[Commit to Flux Repo]
    end
